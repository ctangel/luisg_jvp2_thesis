    ...
    if os.path.isfile(denc_file_name):
        with open(denc_file_name) as f:
            ...
            if digest != m.digest():
                ...
                # Try dev_id
                print "\t\ttrying id..."
                dec = sp.check_output("./decrypt %s < param/a3.param" % (dev_id), shell=True)
                try:
                    data = json.loads(dec)
                except:
                    # Try glob_id
                    print "\t\ttrying global id..."
                    dec = sp.check_output("./decrypt %s < param/a3.param" % (glob_id), shell=True)
                    try:
                        data = json.loads(dec)
                    except:
                        print "\t\tpass"
