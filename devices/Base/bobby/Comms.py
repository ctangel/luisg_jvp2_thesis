...
import Queue
import serial, threading, time, string, binascii, subprocess as sp, os
    ...
    def unpackage(self, data, source, devID):
        ...
        try:
            ...
            if self.destinations[source].get(id) == None:
                print "dank"
                self.destinations[source][id] = MessageBuilder(max)
            print dec_msg
            print dec_msg[1:]
            if self.destinations[source][id].addMessage(dec_msg[1:]):
                print "fuh"
...
class Comms():
    ...
    def sendData(self, dest, data, ack, devID):
        if ack: #This should be encrypted. Luis promised it will be
            ...
            delivery = Delivery()
            for msg in delivery.package(dest, data, devID):
    ...
    def broadcastData(self, data):
        for msg in Delivery.package(dest, data):
    ...
    def readMessage(self):
        ...
        if msg['id'] is 'rx':
            ...
            delivery = Delivery()
            msgID, data = delivery.unpackage(msg['rf_data'], msg['source_addr_long'], dev_id)
            if data != None:
                ...
                msg = Delivery.encrypt(m, senderID)
                self.sendData(dest=dest, data=msg, ack=True)
                return {'rx': data}
            return None
